...
export function Sidebar() {
    ...
    const filteredNavItems = navItems.filter(item => {
        if (isClient) {
            // Clients see: Dashboard, Funil, Agendamentos, Pacientes, Suporte (Conversas hidden due to bugs)
            return ['Dashboard', 'Funil', 'Agendamentos', 'Pacientes', 'Suporte'].includes(item.name)
        } else {
                    ...
                    return (
                        <Button
                            ...
                            className={cn(
                                ...
                                isActive && "bg-primary/10 text-amber-800 dark:text-primary hover:bg-primary/20 hover:text-amber-900 dark:hover:text-primary"
                            )}
                        >
                            <Link href={item.href}>
                                <item.icon className={cn("h-5 w-5", isActive && "text-amber-800 dark:text-primary")} />
