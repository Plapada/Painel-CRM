...
const CalendarGridComponent = ({ isRange = false, bookedDays = new Set() }: { isRange?: boolean, bookedDays?: Set<string> }) => {
                ...
                {(date) => (
                    <CalendarCellRac
                        ...
                        className={cn(
                            ...
                            cn(
                                "before:pointer-events-none before:absolute before:top-1.5 before:right-1.5 before:z-10 before:size-[4px] before:rounded-full before:bg-orange-500",
...
const Calendar = ({ className, bookedDays, ...props }: CalendarProps) => {
    return (
        <CalendarRac
            {...props}
        ...
        >
            <CalendarHeader />
            <CalendarGridComponent bookedDays={bookedDays} />
        </CalendarRac>
    )
}

const RangeCalendar = ({ className, bookedDays, ...props }: RangeCalendarProps) => {
    return (
        <RangeCalendarRac
            {...props}
            className={composeRenderProps(className, (className) =>
        ...
        >
            <CalendarHeader />
            <CalendarGridComponent isRange bookedDays={bookedDays} />
        </RangeCalendarRac>
    )
}
